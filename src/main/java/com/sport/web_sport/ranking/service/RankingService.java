...
public class RankingService {
    ...
    public List<RankingTeamResponse> getRankings(SportType sportType) {
        ...
        for (Map.Entry<Long, int[]> e : acc.entrySet()) {
            ...
            if (sportType == SportType.BASEBALL && !isOfficialMlbFranchise(team)) continue;
    ...
    private static final Pattern MLB_LOGO_TEAM_ID = Pattern.compile("/team/(\\d+)/");

    // Official MLB franchise IDs from mlbstatic.com are in the 108–200 range.
    // WBC national teams and minor/college teams have IDs > 200.
    private boolean isOfficialMlbFranchise(Team team) {
        String logo = team.getLogoUrl();
        if (logo == null) return false;
        Matcher m = MLB_LOGO_TEAM_ID.matcher(logo);
