...
[Spring Boot — soccer 모듈]
  ...
  /api/soccer/fixtures/:id/analysis (Gemini AI 분석)
        │
        ▼
[React 프론트엔드]
  /sports/soccer  (종합 대시보드)
  /soccer/fixtures, /soccer/fixtures/:id, /soccer/standings
  └─ 경기 상세 페이지에 AI 분석 카드 (종료 경기만)
        │
        ▼
[Google Gemini API]
  종료 경기의 요약·전술·핵심 포인트를 한국어로 생성
```
...
### 2-4. Gemini API 키 (AI 분석용)

K리그 경기 상세 페이지의 AI 분석 기능을 사용하려면 Google Gemini API 키가 필요합니다.

1. **AI Studio**(`aistudio.google.com`)에서 발급: <https://aistudio.google.com/apikey>
   - Google **Cloud Console**이 아닌 **AI Studio**에서 받아야 합니다 (다른 사이트).
   - `Create API key` → `Create API key in new project` 권장.
2. 발급된 `AIzaSy...` 키를 환경변수로 설정 (commit 안전):
   ```bash
   export GEMINI_API_KEY=AIzaSy...본인키...
   ./mvnw spring-boot:run
   ```
   또는 `application.properties`에 직접 (commit 전 반드시 복원):
   ```properties
   gemini.api.key=AIzaSy...본인키...
   ```

**`PERMISSION_DENIED` (403) 발생 시**: 발급한 Google 계정이 Gemini 콘텐츠 생성 권한을
가지지 못한 경우입니다. 다른 Google 계정으로 키를 재발급하거나, Cloud 결제 계정을
연결하면 해결됩니다 (한도 내에선 과금되지 않음).

키가 없어도 분석을 제외한 다른 기능(경기 목록·상세·순위)은 정상 동작합니다.
...
│   └── FixtureAnalysis.java   → @Table("SOCCER_FIXTURE_ANALYSIS") AI 분석 캐시
├── repository/
│   ├── SoccerTeamRepository
│   ├── FixtureRepository      (search, findByIdWithTeams, findRecentFinishedByTeam)
│   ├── StandingRepository     (findBySeason, findBySeasonAndTeamId)
...
│       ├── StandingResponse
│       └── FixtureAnalysisResponse   (AiAnalysisCard와 호환되는 필드명)
├── service/
│   ├── FixtureService
│   ├── StandingService
│   ├── SoccerTeamService
│   ├── FixtureAnalysisService          (조회·생성·재생성 오케스트레이션)
│   └── FixtureAnalysisGenerator        (Gemini 프롬프트 빌드 + HTTP 호출)
└── controller/
...
| GET | `/api/soccer/fixtures/{id}/analysis` | `ApiResponse<FixtureAnalysisResponse>` | 저장된 AI 분석 조회 |
| POST | `/api/soccer/fixtures/{id}/analysis/generate` | `ApiResponse<FixtureAnalysisResponse>` | AI 분석 생성 (기존 DONE은 그대로 반환) |
| POST | `/api/soccer/fixtures/{id}/analysis/regenerate` | `ApiResponse<FixtureAnalysisResponse>` | AI 분석 강제 재생성 |
...
`FixtureAnalysisGenerator`는 Gemini에 다음 컨텍스트를 함께 전달합니다:

1. **경기 정보** — 리그·시즌·라운드·홈/원정팀·스코어·경기장·일시·관중
2. **양 팀의 시즌 성적** — `STANDINGS` 테이블에서 현재 순위·승점·승무패·득실
3. **양 팀의 최근 5경기 폼** — `FIXTURES` 테이블에서 같은 시즌 종료 경기 중 최신 5개씩

이 컨텍스트로 Gemini는 단순 스코어 평이 아닌 **시즌 맥락이 담긴 분석**을 생성합니다.
응답 구조 (JSON으로 모델에 요청):

| 필드 | 길이 |
|---|---|
| `summary` | 5~7문장 — 결과·흐름·시즌 맥락 종합 |
| `tactical` | 단락 2개 — 홈팀 전술·강점 / 원정팀 전술·약점 |
| `keyPoint` | "1) ... 2) ... 3) ..." 형식의 핵심 포인트 3가지 |

설정값 (`application.properties`):
- `gemini.model` 기본 `gemini-2.5-flash-lite` — 계정에 따라 `gemini-2.5-flash`로 변경 권장
- `maxOutputTokens` 2048 (코드 상수), `temperature` 0.5

분석 결과는 `SOCCER_FIXTURE_ANALYSIS` 테이블에 캐싱되어 재호출 시 즉시 응답.
`regenerate` 엔드포인트는 캐시를 무시하고 새로 생성.
...
| `/soccer/fixtures/:fixtureId` | `SoccerFixtureDetailPage` | 스코어보드 + 라운드/구장/관중/썸네일 + **AI 분석 카드** (종료 경기만) |
...
frontend/src/api/soccerApi.js
  ...
  - getFixtureAnalysis(fixtureId, signal)
  - generateFixtureAnalysis(fixtureId)
  - regenerateFixtureAnalysis(fixtureId)
```

기존 `axiosInstance`를 그대로 사용하며 `baseURL`은 `http://localhost:8080/api`입니다.

`SoccerFixtureDetailPage`는 기존 `components/AiAnalysisCard.jsx`를 그대로 재사용합니다
(필드명을 동일하게 맞춘 `FixtureAnalysisResponse` 덕분에 호환).
...
### AI 분석 호출 시 `GEMINI_API_KEY 환경변수가 설정되지 않았습니다`
→ 2-4 단계의 키 설정이 빠진 경우. 환경변수 또는 `application.properties` 둘 중 하나로 설정.

### AI 분석 호출 시 `Gemini API 오류 (status=403): Your project has been denied access`
→ 발급한 Google 계정 자체가 Gemini 콘텐츠 생성 권한을 가지지 못한 상태입니다.
   다른 Google 계정으로 키를 재발급하거나 Google Cloud에 결제 계정을 연결하면 해결됩니다.

### AI 분석 호출 시 `Gemini API 오류 (status=429): RESOURCE_EXHAUSTED, limit: 0`
→ 해당 모델의 무료 한도가 0으로 막힌 상태입니다. `application.properties`의
   `gemini.model`을 `gemini-2.5-flash` 또는 `gemini-2.0-flash`로 변경하거나 키 재발급.

---

## 8. 향후 개선 아이디어

- [x] ~~경기 상세에 AI 분석(Gemini) 통합~~ (2026-05-28 완료)
- [ ] TheSportsDB 외에도 K리그 공식 데이터(KFA) 연동
- [ ] 경기 상세에 라인업·이벤트(골·교체·카드) 표시 (TheSportsDB 추가 호출 필요)
...
- [ ] AI 분석 결과를 더 풍부하게 — 양 팀 상대 전적, 득점왕·도움왕 등 컨텍스트 추가
