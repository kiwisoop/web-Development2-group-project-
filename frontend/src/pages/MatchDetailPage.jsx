...
import MlbAnalysisTab from '../components/MlbAnalysisTab';
...
const MLB_TABS = ['팬존', '라인업', '기록', '중계', '존 차트', '분석'];
...
export default function MatchDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState('팬존');
  ...
  useEffect(() => {
    if (!isLoggedIn) { setFavoriteTeams([]); return; }
    const controller = new AbortController();
  ...
  const handleVote = async (voteOption) => {
    ...
    finally { setPredictionVoting(false); }
  };
          ...
          <div className="tab-panel">
            ...
            {activeTab === '라인업' && (
              <div className="card">
                ...
                {mlbDetailError ? (
                  <div style={{ color: 'var(--color-error)', padding: '1rem' }}>{mlbDetailError}</div>
                ) : !mlbDetail ? (
                  <div style={{ color: 'var(--color-text-muted)', padding: '1rem' }}>MLB 데이터를 불러오는 중...</div>
                ) : (
                  <div className="mlb-lineup-grid">
                    <MlbLineupTable lineup={mlbDetail.awayLineup} title={mlbDetail.awayTeamShortName || mlbDetail.awayTeamName} />
                    <MlbLineupTable lineup={mlbDetail.homeLineup} title={mlbDetail.homeTeamShortName || mlbDetail.homeTeamName} />
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === '분석' && (
              <MlbAnalysisTab
                matchId={matchId}
                homeTeam={mlbDetail?.homeTeamShortName || mlbDetail?.homeTeamName || '홈'}
                awayTeam={mlbDetail?.awayTeamShortName || mlbDetail?.awayTeamName || '원정'}
              />
            )}
