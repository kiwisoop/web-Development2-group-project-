...
const INSIGHTS = [
  ['Pitch Zone', '투구 위치, 구종, 타자별 승부 패턴을 한 화면에서 비교합니다.'],
  ['Live Boxscore', '이닝별 득점, 안타, 실책, 선수 스탯을 경기 흐름과 함께 확인합니다.'],
  ['AI Analysis', '승부처, 불펜 기용, 타선 연결성을 짧은 리포트로 요약합니다.'],
];
    ...
    <div className="baseball-page">
      <section className="baseball-hero">
        ...
        <div className="baseball-hero-copy">
          ...
          <h1>MLB 일정, 스코어, 분석으로 바로 들어가는 야구 허브</h1>
          <p>
            오늘 예정된 MLB 경기만 모아서 보여주고, 경기 상세에서 라인스코어와 투구 기록,
            AI 리포트로 바로 이어지도록 정리했습니다.
          </p>
      ...
      <section className="baseball-toolbar" aria-label="야구 빠른 탐색">
        <Link to={`/matches?sportType=BASEBALL&date=${selectedDate}`}>오늘 경기</Link>
        <Link to={`/matches?sportType=BASEBALL&status=SCHEDULED&date=${selectedDate}`}>오늘 예정 경기</Link>
        <Link to="/rankings/baseball">팀 순위</Link>
        <Link to="/analysis?sportType=BASEBALL">분석 리포트</Link>
      </section>
      ...
      <section className="baseball-grid">
        <div className="baseball-main-column">
          ...
          <div className="baseball-game-list">
            {(featuredMatches.length > 0 ? featuredMatches : []).slice(0, 6).map((game) => {
              ...
              <Link key={game.id || `${teamName(game.awayTeam)}-${teamName(game.homeTeam)}`} to={`/matches/${game.id}`} className="baseball-game-card">
                ...
                <p>{game.venue || game.league?.leagueName || 'MLB 경기 상세와 분석을 확인하세요.'}</p>
              </Link>
              );
            })}
            {featuredMatches.length === 0 && (
              <div className="baseball-empty-card">
                ...
                <span>{selectedDate} 기준으로 저장된 야구 경기만 표시합니다.</span>
              </div>
            )}
          </div>
        </div>
