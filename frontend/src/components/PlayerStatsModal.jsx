  ...
  return (
    <div className="lck-pstat-season-card">
      <div className="lck-pstat-season-head">
        <span className="lck-pstat-season-name">{summary.season || '시즌 미상'}</span>
        <span className="lck-pstat-season-games">{fmtNum(summary.totalGames)}경기</span>
      </div>

      <div className="lck-pstat-kda-row">
        <div className="lck-pstat-kda-main">
          <span className="lck-pstat-kda-k">{fmtNum(summary.totalKills)}</span>
          <span className="lck-pstat-kda-sep">/</span>
          <span className="lck-pstat-kda-d">{fmtNum(summary.totalDeaths)}</span>
          <span className="lck-pstat-kda-sep">/</span>
          <span className="lck-pstat-kda-a">{fmtNum(summary.totalAssists)}</span>
        </div>
      ...
      <div className="lck-pstat-stat-grid">
        ...
        <div><span>승</span><strong className="lck-win">{fmtNum(summary.wins)}</strong></div>
        <div><span>패</span><strong className="lck-loss">{fmtNum(summary.losses)}</strong></div>
      </div>
    </div>
  );
}
...
function GameStatsTable({ rows }) {
  if (!rows || rows.length === 0) {
    return <EmptyState title="기록 없음" description="해당 조건의 경기별 상세 지표가 없습니다." />;
  }
          ...
          {rows.map((g) => (
            <tr key={g.gameId} className={g.win ? 'lck-row-win' : 'lck-row-loss'}>
              ...
              <td className="lck-kda-cell">
                <span className="lck-pstat-kda-k">{fmtNum(g.kills)}</span>
                <span> / </span>
                <span className="lck-pstat-kda-d">{fmtNum(g.deaths)}</span>
                <span> / </span>
                <span className="lck-pstat-kda-a">{fmtNum(g.assists)}</span>
              </td>
              <td>{fmtNum(g.cs)}</td>
              <td>{fmtNum(g.gold)}</td>
              <td className="lck-dpm">{fmtNum(g.dpm)}</td>
              <td className="lck-dmg-ratio">{fmtPercent(g.teamDamageRatio)}</td>
              <td>{fmtNum(g.visionScore)}</td>
...
export default function PlayerStatsModal({ player, teamName, onClose }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((e) => {
        if (e?.code !== 'ERR_CANCELED') setError('선수 지표를 불러오지 못했습니다.');
      })
      ...
      <div className="lck-modal" onClick={(e) => e.stopPropagation()}>
        <div className="lck-modal-head">
          <div className="lck-modal-title-row">
            ...
            <h2 className="lck-modal-nick">{player.nickname || player.playerName || '선수'}</h2>
            ...
            <>
              {career && career.totalGames > 0 && (
                <div className="lck-career-bar">
                  <span>통산</span>
                  <strong>{fmtNum(career.totalGames)}경기</strong>
                  <span className="lck-win">{fmtNum(career.wins)}승</span>
                  <span className="lck-loss">{fmtNum(career.losses)}패</span>
                  <span>승률 <strong>{fmtPercent(career.winRate)}</strong></span>
                </div>
              )}
              ...
              <section className="lck-modal-section">
                ...
                {summaries.length === 0 ? (
                  ...
                  <div className="lck-pstat-season-grid">
                    {summaries.map((s) => (
                      <SeasonSummaryCard key={s.season || s.id} summary={s} />
                    ))}
                  </div>
                )}
              </section>
              ...
              <section className="lck-modal-section">
                ...
                <p className="lck-table-hint">
                  DPM, 골드, 시야 점수, 팀 데미지 비율은 API에 값이 있을 때만 표시됩니다.
                </p>
